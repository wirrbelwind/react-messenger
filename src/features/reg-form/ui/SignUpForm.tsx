...
export const SignUpForm = (props: SignUpFormProps) => {
	const { control, handleSubmit, watch, formState: { errors } } = useForm<IRegFormTemplate>({ defaultValues: { email: '', name: '', pwd: '', confirmPwd: '', photoURL: '', photoBase64: '' } })
	const [createUserWithEmailAndPassword, user, loading, signUpError] = userModel.useCreateUserEmailPwd()
	...
	return (<AuthFormContainer>
		...
		<Typography variant="subtitle1" component={Link} to='/signin' color='primary.dark' fontSize={18}>
			Don't have an account? Sign Up!
		</Typography>
		<SubmitBtn onClick={handleSubmit(onSubmit)} loading={loading} disabled={loading}>
			Create account
		</SubmitBtn>
	</AuthFormContainer>)
}
