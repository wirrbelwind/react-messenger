	...
	return (<Layout>
		...
		{regularMessages &&
			regularMessages.map(msg => {
				return (<MessageEntity message={msg as IMessage} key={msg.timestamp.toMillis()} owned={msg.sender.id === user.uid} />)
			})}
		{messagesQueue?.map(msg => <MessageEntity message={msg} key={msg.timestamp.toMillis()} isPending={true} />)}
